...
__global__  void atomic_histo(unsigned int *d_data,unsigned int *d_bins)
{
    unsigned int threadId = threadIdx.x + blockDim.x*blockIdx.x;
    unsigned int item = d_data[threadId];
    atomicAdd(&(d_bins[item]),1);
}
...
void histo(unsigned int* d_data,unsigned int* d_bins)
{
    ...
    unsigned int blocks = ARRAY_SIZE/threads;
    atomic_histo<<<blocks,threads>>>(d_data,d_bins);
